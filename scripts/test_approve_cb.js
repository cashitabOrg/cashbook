...
async function main() {
  ...
  const sessionId = 'cb8d3f1c-2eed-43ea-9e7e-b7ac9235f197';
  const adminId = '8faaee25-598f-4a72-a21d-c3ebaf20bf0b';

  console.log(`Checking session ${sessionId} state before...`);
  ...
  const { data: after, error: updateErr } = await supabase
    ...
    .update({
      ...
      approved_by: adminId,
      approval_reason: 'Testing cb session'
    })
    .eq('id', sessionId)
    .select();
