      ...
      <div className="flex-1 overflow-auto h-[400px] lg:max-h-[calc(100vh-320px)]">
        <table className="min-w-full divide-y divide-slate-100 dark:divide-[#2C2C2E]">
          <thead className="bg-slate-50/80 dark:bg-[#252528]/80 sticky top-0 z-10 text-[10px] font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest">
            <tr>
              <th className="py-2.5 px-2 text-center w-8">SN</th>
          ...
          <tbody className="bg-white dark:bg-[#1C1C1E] divide-y divide-slate-100 dark:divide-[#2C2C2E]">
            {filteredInventory.length === 0 ? (
              <tr>
                <td colSpan={4} className="py-10 text-center text-sm text-slate-400 dark:text-gray-500 font-medium italic">No products found matching search.</td>
              </tr>
            ) : (
              filteredInventory.map((p, idx) => {
                ...
                return (
                  <tr key={p.id} className={`transition-colors hover:bg-slate-50/50 dark:hover:bg-[#252528]/50 ${isLow ? 'bg-red-50/30 dark:bg-red-500/10' : ''}`}>
                    <td className="py-2.5 px-2 text-[10px] text-slate-400 dark:text-gray-500 font-mono italic text-center">{idx + 1}</td>
                    <td className="py-2.5 px-3 text-sm font-bold text-slate-900 dark:text-white">{p.name}</td>
                    ...
                    <td className="py-2.5 px-3 text-center">
                      {isLow ? (
                        ...
                        <span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-500/20 px-2 py-0.5 rounded uppercase tracking-tighter">Ok</span>
                      )}
                    </td>
                  </tr>
                );
