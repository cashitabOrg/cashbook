        ...
        <div className="p-2 lg:p-4 bg-gray-50/50 dark:bg-[#252528]/30 animate-in slide-in-from-top-2 duration-300 border-t border-gray-200 dark:border-[#2C2C2E]">
          ...
          {sessionsList.length > 0 && (
            <div className="mb-6 bg-gray-100/30 dark:bg-[#1E1E20]/50 border border-gray-200/50 dark:border-[#2C2C2E] p-4 rounded-2xl shadow-sm">
              ...
              <div className="flex flex-row overflow-x-auto gap-3 pb-1 scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                {sessionsList.map((session: any) => {
                  const isSessApproved = session.approvalStatus === 'approved';
                  return (
                    <div 
                      key={session.id}
                      className="bg-white dark:bg-[#1C1C1E] border border-gray-150 dark:border-[#2C2C2E] p-3.5 rounded-xl shadow-sm flex flex-col justify-between transition-all hover:shadow-md hover:scale-[1.01] hover:border-gray-300 dark:hover:border-[#3A3A3C] duration-200 w-64 flex-shrink-0"
                    >
                      <div>
