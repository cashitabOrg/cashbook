           ...
           <tbody className="divide-y divide-gray-100 dark:divide-[#2C2C2E]">
              {sortedIntel.map((stats) => (
                <tr key={stats.name} className="text-[11px] hover:bg-gray-50 dark:hover:bg-[#2C2C2E]/50 transition-colors">
                  ...
                  <td className="py-3 px-4 text-right text-gray-400 dark:text-gray-500 font-mono italic">₦{stats.unitPrice.toFixed(2)}</td>
                  <td className="py-3 px-4 text-right font-bold text-gray-900 dark:text-gray-200">₦{stats.recordedRevenue.toFixed(2)}</td>
